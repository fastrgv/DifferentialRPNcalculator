...
-- total differential 
dnum := right*ddll+left*ddrr; --product rule
...
-- total differential 
dnum := num* ( log(left)*ddrr + right/left*ddll );
...
when sin =>
	...
	num:=sin(right);
	dnum:=ddrr*cos(right); --differential for SIN
	push( data );

when cos =>
	if degmode then right:=right*deg2rad; end if;
	num:=cos(right);
	dnum:=ddrr*sin(right); --differential for COS
	push( data );
...
when ln =>
	num:=log(right);
	dnum:=ddrr/right; --differential for LOG
	push( data );
